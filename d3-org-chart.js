  ...
  class OrgChart {
    constructor() {
      ...
      const attrs = {
          ...
          return `<defs>
                  ${visibleConnections
                    .map(conn => {
                     ...
                     <marker id="arrow-${conn.from +
                       '_' +
                       conn.to}"  markerWidth="500"  markerHeight="500"  refY="2"  refX="1" orient="${
                        conn._source.x < conn._target.x
                          ? 'auto'
                          : 'auto-start-reverse'
                      }" >
        ...
        // },
        nodeContent: d => `<div style="padding:5px;font-size:10px;">Sample Node(id=${
          d.id
        ...
        layoutBindings: {
          ...
          bottom: {
            ...
            centerTransform: ({
              ...
              `translate(${centerX},${chartHeight -
                rootMargin}) scale(${scale})`,
            nodeFlexSize: ({
              height,
              width,
              siblingsMargin,
          ...
          right: {
            ...
            centerTransform: ({
              ...
              `translate(${chartWidth -
                rootMargin},${centerY}) scale(${scale})`,
            nodeFlexSize: ({
              height,
              width,
              siblingsMargin,
    ...
    render() {
      ...
      attrs.flexTreeLayout = d3Flextree
        .flextree({
          nodeSize: node => {
        ...
        .spacing((nodeA, nodeB) =>
          nodeA.parent == nodeB.parent
            ? 0
            : attrs.neightbourMargin(nodeA, nodeB)
        );
    ...
    calculateCompactFlexDimensions(root) {
      ...
      root.eachBefore(node => {
        if (node.children && node.children.length > 1) {
          ...
          const rowsMap = d3.rollup(
            compactChildren,
            reducedGroup =>
              d3.max(
                reducedGroup,
                d =>
                  attrs.layoutBindings[attrs.layout].compactDimension.sizeRow(
                    d
      ...
      root.eachBefore(node => {
        if (node.children) {
          ...
          const cumSum = d3.cumsum(
            [...rowsMap].map(d => d[1] + attrs.compactMarginBetween(d))
          );
    ...
    update({ x0, y0, x, y, width, height }) {
      ...
      const visibleConnections = connections.filter(
        d => visibleNodesMap.has(d.from) && visibleNodesMap.has(d.to)
      );
        ...
        .attr('d', d => {
          const n =
            attrs.compact && d.flexCompactDim
              ? {
                  x: attrs.layoutBindings[attrs.layout].compactLinkMidX(
                    d,
                    attrs
                  ),
                  y: attrs.layoutBindings[attrs.layout].compactLinkMidY(
                    d,
                    attrs
                  )
                }
